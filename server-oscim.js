...
function loadTile(z, x, y, callback) {
    var filename = './tiles-oscim/' + z + '-' + x + '-' + y + '.vtm';
    fs.readFile(filename, function(err, data) {
        if (err) {
            request({
                url: 'http://opensciencemap.org/tiles/vtm/' + z + '/' + x + '/' + y + '.vtm',
                encoding: null
...
function convertTile(z, x, y, callback) {
    ...
    async.waterfall([
        ...
        function(data, callback) {
            tile = new fontserver.TileOSciM(data);
            var after = tile.serialize();
            zlib.deflate(after, callback);
        }, function(data, callback) {
            fs.writeFile('./tiles-oscim/' + z + '-' + x + '-' + y + '.vector.pbf', data, function(err) {
                callback(err, data);
            });
        }
    ], callback);
...
app.get('/gl/tiles/:z(\\d+)-:x(\\d+)-:y(\\d+).vector.pbf', function(req, res) {
    ...
    fs.readFile('./tiles-oscim/' + z + '-' + x + '-' + y + '.vector.pbf', function(err, data) {
        if (err) {
...
async.each(['tiles-oscim'], mkdirp, function(err) {
    if (err) throw err;
    app.listen(3333);
